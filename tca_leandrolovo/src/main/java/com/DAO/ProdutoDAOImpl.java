...
public class ProdutoDAOImpl implements ProdutoDAO {
    ...
    public boolean editarProduto(Produtos produto, String coluna, int tipo) throws SQLException {
        if (produto == null || coluna == null || coluna.isEmpty()) {
            throw new IllegalArgumentException("Produto ou coluna inválidos.");
        }

        String sql = null;

        switch (tipo) {
            case 1: // Atualizar coluna
                sql = "UPDATE Produto SET " + coluna + " = ? WHERE id_produto = ?";
                break;
            case 2: // Entrada no estoque
                sql = "INSERT INTO Estoque (fk_id_produto, quantidade, tipo_movimentacao, data_movimentacao) " +
                        "VALUES (?, ?, 'Entrada', CURRENT_TIMESTAMP)";
                break;
            case 3: // Saída no estoque
                sql = "INSERT INTO Estoque (fk_id_produto, quantidade, tipo_movimentacao, data_movimentacao) " +
        ...
        try (Connection con = FabricaConexao.faz_Conexao(); PreparedStatement stmt = con.prepareStatement(sql)) {
            ...
            Logger.getLogger(FabricaConexao.class.getName()).log(Level.SEVERE, null, e);
            JOptionPane.showMessageDialog(null, "Erro ao editar produto: " + e.getMessage(), "Erro",
